const orderWeight = s => s.split(' ').sort((a, b) => a.split('').reduce((pv, cv) => pv + +cv, 0) - b.split('').reduce((pv, cv) => pv + +cv, 0) || (a > b ? 1 : -1)).join(' ');

// const orderWeight = str =>
//   str.split(' ').map((n) => {
//     const weight = n.split('').reduce((a, b) => Number(a) + Number(b), 0)
//     return {n, weight}
//   }).sort((a, b) => a.weight === b.weight ? a.n.localeCompare(b.n) : a.weight - b.weight).map(({n}) => n).join(' ')
